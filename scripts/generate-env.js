...
function main() {
  ...
  if (fs.existsSync(envPath) && !force) {
    console.log('⚠️  .env.production already exists. Skipping generation to prevent overwriting.');
    console.log('If you want to generate a new one, run with the --force flag or delete the existing file.');
    process.exit(0);
  }
  ...
  try {
    console.log('Generating Caddy bcrypt hash for dashboard password using Docker...');
    ...
    console.warn('⚠️  Could not run Docker to generate Caddy password hash. You will need to hash it manually.');
    console.warn('Plaintext password: ' + dashboardPassword);
    console.warn('To generate the hash, run: docker run --rm caddy:2-alpine caddy hash-password --plaintext "' + dashboardPassword + '"');
...
# Alchemy / EVM Node RPC URL (replace with your RPC URL)
